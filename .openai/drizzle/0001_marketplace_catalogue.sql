...
CREATE INDEX IF NOT EXISTS idx_agents_current_network_evidence ON agents(current, network, evidence_rank, observed_at);
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS idx_agents_current_name ON agents(current, normalized_name, agent_id);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE VIRTUAL TABLE IF NOT EXISTS agents_fts USING fts5(name, description, search_text, content='agents', content_rowid='rowid', tokenize='porter unicode61');
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS agents_fts_insert AFTER INSERT ON agents BEGIN
  INSERT INTO agents_fts(rowid, name, description, search_text) VALUES (new.rowid, new.name, new.description, new.search_text);
END;
--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS agents_fts_delete AFTER DELETE ON agents BEGIN
  INSERT INTO agents_fts(agents_fts, rowid, name, description, search_text) VALUES ('delete', old.rowid, old.name, old.description, old.search_text);
END;
--> statement-breakpoint
